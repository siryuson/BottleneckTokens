#!/usr/bin/env python
"""CLI wrapper for TextVQA training Lance conversion."""
...
from vlm2emb.data.conversion.textvqa_train import (
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--raw-root", type=Path, required=True, help="Local TextVQA parquet root")
    parser.add_argument("--output", type=Path, required=True, help="Converted Lance root")
    parser.add_argument("--eval-root", type=Path, default=None, help="Optional MMEB-V2 TextVQA eval root")
...
def main() -> int:
    """Run the TextVQA conversion wrapper."""

    args = parse_args()
    if args.ensure_indices_only:
        ensure_textvqa_train_indices(args.output)
        return 0
    summary = write_textvqa_train_root(
