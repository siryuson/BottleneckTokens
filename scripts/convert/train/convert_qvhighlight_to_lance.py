#!/usr/bin/env python
"""CLI wrapper for QVHighlights training Lance conversion."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--raw-root", type=Path, required=True, help="Local QVHighlights annotation root")
    parser.add_argument("--video-root", type=Path, required=True, help="Directory containing QVHighlights mp4 files")
    ...
    parser.add_argument("--num-workers", type=int, default=8, help="Parallel workers for video rows")
...
def main() -> int:
    """Run the QVHighlights conversion wrapper."""

    args = parse_args()
    if args.ensure_indices_only:
        ensure_qvhighlight_indices(args.output)
        return 0
    summary = write_qvhighlight_root(
