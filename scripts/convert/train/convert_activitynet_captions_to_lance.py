#!/usr/bin/env python
"""CLI wrapper for ActivityNet Captions training Lance conversion."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--source-root", type=Path, help="Local friedrichor/ActivityNet_Captions root")
    parser.add_argument(
        "--video-archive",
        type=Path,
        action="append",
        dest="video_archives",
        help="ActivityNet_Videos tar part; pass multiple times in part order",
    )
    ...
    parser.add_argument("--split-batch-size", type=int, default=4096, help="Segment rows per Lance write batch")
    parser.add_argument("--segment-max-frames", type=int, default=8, help="Maximum sampled frames per segment")
    parser.add_argument(
        "--max-bytes-per-file",
        type=int,
...
def main() -> int:
    """Run the ActivityNet Captions conversion wrapper."""

    args = parse_args()
    if args.ensure_indices_only:
        ensure_activitynet_captions_indices(args.output)
        return 0
    ...
    summary = write_activitynet_captions_root(
