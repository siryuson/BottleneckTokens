...
def dist_gather(x: torch.Tensor) -> torch.Tensor:
...
@torch.no_grad()
def dist_gather_nograd(x: torch.Tensor) -> torch.Tensor:
    if not dist.is_initialized():
        return x
...
@torch.no_grad()
def varsize_gather_nograd(x: torch.Tensor) -> torch.Tensor:
    ...
    max_size = max(int(size_tensor.item()) for size_tensor in allsizes)
    ...
    output = [tensor[: int(allsizes[k].item())] for k, tensor in enumerate(output)]
