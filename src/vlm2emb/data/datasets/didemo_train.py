"""DiDeMo training dataset runtime."""
...
DEFAULT_QUERY_INSTRUCTION = "Find a video that includes the following described scenes:"
...
class DidemoTransformConfig:
    """Default transform configuration for DiDeMo training rows."""
...
def _normalize_transform_kwargs(values: Mapping[str, Any] | None) -> DidemoTransformConfig:
    """Validate YAML-friendly default transform kwargs."""

    normalized = normalize_side_transform_mapping(
        values,
        dataset_name="DiDeMo",
        allowed_keys=DIDEMO_TRANSFORM_KEYS,
    )
    ...
    if positive_visual_token_placement != "own_line":
        raise ValueError("DiDeMo transform.positive.visual_token_placement must be 'own_line'.")
    empty = negative.get("empty")
    if empty is not None and empty != "empty_multimodal_input":
        raise ValueError("DiDeMo transform.negative.empty must be 'empty_multimodal_input'.")
    for side, side_values in (("query", query), ("positive", positive), ("negative", negative)):
        ...
        if trailing_newline is not None and trailing_newline not in TRAILING_NEWLINE_VALUES:
            raise ValueError(
                f"DiDeMo transform.{side}.trailing_newline must be one of {sorted(TRAILING_NEWLINE_VALUES)}."
            )
    return DidemoTransformConfig(
...
def build_didemo_query_text(
    ...
    *,
    ...
    """Build the DiDeMo text-to-video query text."""

    body = normalize_text_whitespace(str(caption or ""))
    text = join_instruction_body(instruction, body, separator=instruction_body_separator)
    return _apply_trailing_newline(text, trailing_newline, dataset_name="DiDeMo")


def build_didemo_positive_text(
    *,
    instruction: str = DEFAULT_POSITIVE_INSTRUCTION,
    trailing_newline: str = "ensure_single",
) -> str:
    """Build the DiDeMo positive video text."""
...
def _text_side(text: str, trailing_newline: str) -> dict[str, Any]:
    ...
    return {
        "text": _apply_trailing_newline(normalized, trailing_newline, dataset_name="DiDeMo"),
        "media": [],
    }
...
def _build_video_media(record: Mapping[str, Any], *, num_frames: int) -> list[dict[str, Any]]:
    ...
    if not images:
        raise ValueError("DiDeMo row resolved to zero sampled video frames.")
    return [{"kind": "video", "content": images, "metadata": media_metadata}]


def transform_didemo_train_sample(
    record: dict[str, Any],
    *,
    dataset_name: str,
    split: str,
    num_frames: int,
    config: DidemoTransformConfig,
) -> dict[str, Any]:
    """Transform one joined DiDeMo row into a TrainSample."""
    ...
    return {
        ...
        "positive": {
            "text": build_didemo_positive_text(
...
def build_didemo_train_default_transform(
    *,
    dataset_name: str,
    split: str,
    num_frames: int,
    ...
    """Build the pickle-safe default DiDeMo transform."""

    return partial(
        transform_didemo_train_sample,
        dataset_name=dataset_name,
        split=split,
...
@AutoDataset.register("didemo_train")
class DidemoTrainDataset(SampleLanceDataset):
    """DiDeMo text-to-video training dataset over raw split rows."""
    ...
    def __init__(
        ...
        *,
        ...
        if transform is not None and not callable(transform):
            raise TypeError("DidemoTrainDataset transform must be a callable SampleTransform or None.")
        ...
        effective_transform = (
            ...
            else build_didemo_train_default_transform(
                dataset_name=dataset_name,
                split=split,
                num_frames=num_frames,
    ...
    @classmethod
    def from_config(cls, config: Mapping[str, Any]) -> DidemoTrainDataset:
        ...
        if transform_config is not None and not callable(transform_config):
            if not isinstance(transform_config, Mapping):
                raise TypeError("DiDeMo config transform must be mapping, callable, or None.")
            kwargs.pop("transform")
            if "transform_kwargs" in kwargs:
                raise ValueError("DiDeMo config cannot set both transform and transform_kwargs.")
...
__all__ = [
    ...
    "DidemoTrainDataset",
    "build_didemo_positive_text",
    "build_didemo_query_text",
    "build_didemo_train_default_transform",
    "transform_didemo_train_sample",
]
