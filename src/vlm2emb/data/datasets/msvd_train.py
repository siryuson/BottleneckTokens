...
DEFAULT_QUERY_INSTRUCTION = "Find the video snippet that corresponds to the given summary:"
...
class MsvdTransformConfig:
    """Default transform configuration for MSVD training rows."""
...
def _normalize_transform_kwargs(values: Mapping[str, Any] | None) -> MsvdTransformConfig:
    """Validate YAML-friendly default transform kwargs."""

    normalized = normalize_side_transform_mapping(
        values,
        dataset_name="MSVD",
        allowed_keys=MSVD_TRANSFORM_KEYS,
    )
    ...
    if positive_visual_token_placement != "own_line":
        raise ValueError("MSVD transform.positive.visual_token_placement must be 'own_line'.")
    empty = negative.get("empty")
    if empty is not None and empty != "empty_multimodal_input":
        raise ValueError("MSVD transform.negative.empty must be 'empty_multimodal_input'.")
    for side, side_values in (("query", query), ("positive", positive), ("negative", negative)):
        ...
        if trailing_newline is not None and trailing_newline not in TRAILING_NEWLINE_VALUES:
            raise ValueError(
                f"MSVD transform.{side}.trailing_newline must be one of {sorted(TRAILING_NEWLINE_VALUES)}."
            )
    query_caption_selection = query.get("caption_selection", "first")
    if query_caption_selection not in CAPTION_SELECTION_VALUES:
        raise ValueError(
            f"MSVD transform.query.caption_selection must be one of {sorted(CAPTION_SELECTION_VALUES)}."
        )
    return MsvdTransformConfig(
...
def _select_caption(value: Any, *, selection: str) -> str:
    ...
    raise ValueError(f"MSVD caption_selection must be one of {sorted(CAPTION_SELECTION_VALUES)}.")


def build_msvd_query_text(
    caption: Any,
    *,
    ...
    """Build the MSVD text-to-video query text."""
    ...
    return _apply_trailing_newline(text, trailing_newline, dataset_name="MSVD")


def build_msvd_positive_text(
    *,
    instruction: str = DEFAULT_POSITIVE_INSTRUCTION,
    trailing_newline: str = "ensure_single",
) -> str:
    """Build the MSVD positive video text."""
...
def _text_side(text: str, trailing_newline: str) -> dict[str, Any]:
    ...
    return {
        "text": _apply_trailing_newline(normalized, trailing_newline, dataset_name="MSVD"),
        "media": [],
    }
...
def _build_video_media(record: Mapping[str, Any], *, num_frames: int) -> list[dict[str, Any]]:
    ...
    if not images:
        raise ValueError("MSVD row resolved to zero sampled video frames.")
    return [{"kind": "video", "content": images, "metadata": media_metadata}]


def transform_msvd_train_sample(
    record: dict[str, Any],
    *,
    dataset_name: str,
    split: str,
    num_frames: int,
    config: MsvdTransformConfig,
) -> dict[str, Any]:
    """Transform one joined MSVD row into a TrainSample."""
...
def build_msvd_train_default_transform(
    *,
    dataset_name: str,
    split: str,
    num_frames: int,
    ...
    """Build the pickle-safe default MSVD transform."""

    return partial(
        transform_msvd_train_sample,
        dataset_name=dataset_name,
        split=split,
...
@AutoDataset.register("msvd_train")
class MsvdTrainDataset(SampleLanceDataset):
    """MSVD text-to-video training dataset over raw split rows."""
    ...
    def __init__(
        ...
        *,
        ...
        if transform is not None and not callable(transform):
            raise TypeError("MsvdTrainDataset transform must be a callable SampleTransform or None.")
        ...
        effective_transform = (
            ...
            else build_msvd_train_default_transform(
                dataset_name=dataset_name,
                split=split,
                num_frames=num_frames,
    ...
    @classmethod
    def from_config(cls, config: Mapping[str, Any]) -> MsvdTrainDataset:
        ...
        if transform_config is not None and not callable(transform_config):
            if not isinstance(transform_config, Mapping):
                raise TypeError("MSVD config transform must be mapping, callable, or None.")
            kwargs.pop("transform")
            if "transform_kwargs" in kwargs:
                raise ValueError("MSVD config cannot set both transform and transform_kwargs.")
...
__all__ = [
    ...
    "MsvdTrainDataset",
    "build_msvd_positive_text",
    "build_msvd_query_text",
    "build_msvd_train_default_transform",
    "transform_msvd_train_sample",
]
