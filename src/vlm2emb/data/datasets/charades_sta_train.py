"""Charades-STA training dataset runtime."""
...
class CharadesStaTransformConfig:
    """Default transform configuration for Charades-STA training rows."""
...
def _normalize_transform_kwargs(values: Mapping[str, Any] | None) -> CharadesStaTransformConfig:
    """Validate YAML-friendly default transform kwargs."""

    normalized = normalize_side_transform_mapping(
        values,
        dataset_name="Charades-STA",
        allowed_keys=CHARADES_STA_TRANSFORM_KEYS,
    )
    ...
    if empty is not None and empty != "empty_multimodal_input":
        raise ValueError("Charades-STA transform.negative.empty must be 'empty_multimodal_input'.")
    for side, side_values in (("query", query), ("positive", positive), ("negative", negative)):
        ...
        if trailing_newline is not None and trailing_newline not in TRAILING_NEWLINE_VALUES:
            raise ValueError(
                f"Charades-STA transform.{side}.trailing_newline must be one of "
                f"{sorted(TRAILING_NEWLINE_VALUES)}."
            )
    return CharadesStaTransformConfig(
...
def _media_text(text: str, *, trailing_newline: str) -> str:
    ...
    return _apply_trailing_newline(normalized, trailing_newline, dataset_name="Charades-STA")


def build_charades_sta_query_text(
    description: Any,
    *,
    ...
    """Build the Charades-STA full-video query text."""

    body = normalize_text_whitespace(str(description or ""))
...
def build_charades_sta_positive_text(
    *,
    instruction: str = DEFAULT_POSITIVE_INSTRUCTION,
    trailing_newline: str = "ensure_single",
) -> str:
    """Build the Charades-STA localized-clip positive text."""
...
def _text_side(text: str, trailing_newline: str) -> dict[str, Any]:
    ...
    return {
        "text": _apply_trailing_newline(normalized, trailing_newline, dataset_name="Charades-STA"),
        "media": [],
    }
...
def _build_query_media(record: Mapping[str, Any], *, num_frames: int) -> list[dict[str, Any]]:
    ...
    if record.get("id") is not None:
        media_metadata["video_id"] = str(record["id"])
    if not images:
        raise ValueError("Charades-STA row resolved to zero sampled query video frames.")
...
def _build_positive_media(record: Mapping[str, Any], *, num_frames: int) -> list[dict[str, Any]]:
    """Read and sample the positive-side segment frame-unit."""

    clip_start = float(record.get("start_time", 0.0) or 0.0)
    clip_end = float(record.get("end_time", 0.0) or 0.0)
    images, media_metadata = sample_prefixed_preextracted_frame_unit(
        record,
        prefix="positive",
        num_frames=num_frames,
    ...
    if record.get("id") is not None:
        media_metadata["video_id"] = str(record["id"])
    media_metadata["clip_start"] = clip_start
    media_metadata["clip_end"] = clip_end
    if not images:
        raise ValueError("Charades-STA row resolved to zero sampled positive clip frames.")
    return [{"kind": "video", "content": images, "metadata": media_metadata}]
...
def transform_charades_sta_train_sample(
    record: dict[str, Any],
    *,
    dataset_name: str,
    split: str,
    num_frames: int,
    config: CharadesStaTransformConfig,
) -> dict[str, Any]:
    """Transform one joined Charades-STA row into a TrainSample."""
    ...
    return {
        "query": {
            "text": build_charades_sta_query_text(
        ...
        "positive": {
            "text": build_charades_sta_positive_text(
...
def build_charades_sta_train_default_transform(
    *,
    dataset_name: str,
    split: str,
    num_frames: int,
    ...
    """Build the pickle-safe default Charades-STA transform."""

    return partial(
        transform_charades_sta_train_sample,
        dataset_name=dataset_name,
        split=split,
        num_frames=num_frames,
...
@AutoDataset.register("charades_sta_train")
class CharadesStaTrainDataset(SampleLanceDataset):
    """Charades-STA video moment retrieval dataset over raw split rows."""
    ...
    def __init__(
        ...
        *,
        ...
        if transform is not None and not callable(transform):
            raise TypeError("CharadesStaTrainDataset transform must be a callable SampleTransform or None.")
        ...
        effective_transform = (
            ...
            else build_charades_sta_train_default_transform(
                dataset_name=dataset_name,
                split=split,
                num_frames=num_frames,
    ...
    @classmethod
    def from_config(cls, config: Mapping[str, Any]) -> CharadesStaTrainDataset:
        ...
        if transform_config is not None and not callable(transform_config):
            if not isinstance(transform_config, Mapping):
                raise TypeError("Charades-STA config transform must be mapping, callable, or None.")
            kwargs.pop("transform")
            if "transform_kwargs" in kwargs:
                raise ValueError("Charades-STA config cannot set both transform and transform_kwargs.")
...
__all__ = [
    ...
    "build_charades_sta_positive_text",
    "build_charades_sta_query_text",
    "build_charades_sta_train_default_transform",
    "transform_charades_sta_train_sample",
]
