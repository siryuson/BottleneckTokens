...
class GqaTrainTransformConfig:
    """Default transform configuration for GQA training rows."""
...
def _normalize_transform_kwargs(values: Mapping[str, Any] | None) -> GqaTrainTransformConfig:
    """Validate YAML-friendly default transform kwargs."""

    normalized = normalize_side_transform_mapping(values, dataset_name="GQA", allowed_keys=GQA_TRANSFORM_KEYS)
    ...
    if query.get("visual_token_placement") not in {None, "own_line"}:
        raise ValueError("GQA transform.query.visual_token_placement must be 'own_line'.")
    for side, side_values in (("query", query), ("positive", positive), ("negative", negative)):
        ...
        if trailing_newline is not None and trailing_newline not in TRAILING_NEWLINE_VALUES:
            raise ValueError(f"GQA transform.{side}.trailing_newline must be one of {sorted(TRAILING_NEWLINE_VALUES)}.")
    empty = negative.get("empty")
    if empty is not None and empty != "empty_multimodal_input":
        raise ValueError("GQA transform.negative.empty must be 'empty_multimodal_input'.")
    return GqaTrainTransformConfig(
        ...
        query_instruction_body_separator=normalize_instruction_body_separator(
            ...
            dataset_name="GQA transform.query",
            default="space",
        ),
...
def _build_query_text(
    *,
    question: str,
    config: GqaTrainTransformConfig,
) -> str:
    """Build the GQA query text from the raw question."""
    ...
    return _apply_trailing_newline(text, config.query_trailing_newline, dataset_name="GQA")


def _build_query_media(record: Mapping[str, Any]) -> list[dict[str, Any]]:
    """Decode the joined GQA image bytes."""

    image = record.get("image")
    if not isinstance(image, (bytes, bytearray)):
        raise TypeError("GQA row is missing required image bytes.")
    return [
        {
            "kind": "image",
            "content": decode_image(bytes(image)),
            "metadata": {"path": str(record.get("imageId", "") or "")},
        }
    ]


def transform_gqa_train_sample(
    record: dict[str, Any],
    *,
    dataset_name: str,
    split: str,
    ...
    """Transform one joined GQA row into a TrainSample."""

    positive_text = normalize_text_whitespace(str(record.get("fullAnswer", "") or ""))
    return {
        "query": {
        ...
        "metadata": {
            ...
            "imageId": str(record.get("imageId", "") or ""),
            "answer": str(record.get("answer", "") or ""),
            "isBalanced": record.get("isBalanced"),
...
def build_gqa_train_default_transform(
    *,
    dataset_name: str,
    split: str,
    transform_kwargs: Mapping[str, Any] | None = None,
) -> SampleTransform:
    """Build the pickle-safe default GQA transform."""

    return partial(
        transform_gqa_train_sample,
        dataset_name=dataset_name,
        split=split,
...
@AutoDataset.register("gqa_train")
class GqaTrainDataset(SampleLanceDataset):
    """GQA image-question-to-answer training dataset."""

    def __init__(
        self,
        path: str,
        split: str = "official_train_balanced_without_mmeb_v2_eval",
        ...
        *,
        ...
        if transform is not None and not callable(transform):
            raise TypeError("GqaTrainDataset transform must be a callable SampleTransform or None.")
        if read_columns is not None and columns is not None:
            raise ValueError("Pass only one of read_columns or columns for GqaTrainDataset.")
        ...
        effective_transform = (
            ...
            else build_gqa_train_default_transform(
                dataset_name=dataset_name,
                split=split,
    ...
    @classmethod
    def from_config(cls, config: dict[str, Any]) -> GqaTrainDataset:
        """Build a GQA dataset from one YAML config entry."""

        return cls(
            path=str(config["path"]),
            split=str(config.get("split", "official_train_balanced_without_mmeb_v2_eval")),
            dataset_name=str(config.get("dataset_name", "GQA")),
...
__all__ = [
    "GqaTrainDataset",
    "GqaTrainTransformConfig",
    "build_gqa_train_default_transform",
    "transform_gqa_train_sample",
]
