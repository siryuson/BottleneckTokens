"""UCF101 training dataset runtime."""
...
DEFAULT_QUERY_INSTRUCTION = "What activities or sports are being performed by the person in the video?"
TRAILING_NEWLINE_VALUES = {"preserve", "strip", "ensure_single"}
UCF101_TRANSFORM_KEYS = {
...
class Ucf101TransformConfig:
    """Default transform configuration for UCF101 training rows."""
...
def _normalize_transform_kwargs(values: Mapping[str, Any] | None) -> Ucf101TransformConfig:
    """Validate YAML-friendly default transform kwargs."""

    normalized = normalize_side_transform_mapping(
        values,
        dataset_name="UCF101",
        allowed_keys=UCF101_TRANSFORM_KEYS,
    )
    ...
    if visual_token_placement != "own_line":
        raise ValueError("UCF101 transform.query.visual_token_placement must be 'own_line'.")
    empty = negative.get("empty")
    if empty is not None and empty != "empty_multimodal_input":
        raise ValueError("UCF101 transform.negative.empty must be 'empty_multimodal_input'.")
    for side, side_values in (("query", query), ("positive", positive), ("negative", negative)):
        ...
        if trailing_newline is not None and trailing_newline not in TRAILING_NEWLINE_VALUES:
            raise ValueError(
                f"UCF101 transform.{side}.trailing_newline must be one of {sorted(TRAILING_NEWLINE_VALUES)}."
            )
    instruction = query.get("instruction", DEFAULT_QUERY_INSTRUCTION)
    if not isinstance(instruction, str) or not instruction.strip():
        raise ValueError("UCF101 transform.query.instruction must be a non-empty string.")
    return Ucf101TransformConfig(
...
def build_ucf101_query_text(
    *,
    instruction: str = DEFAULT_QUERY_INSTRUCTION,
    trailing_newline: str = "ensure_single",
) -> str:
    """Build the UCF101 video classification query text."""
    ...
    return _apply_trailing_newline(text, trailing_newline, dataset_name="UCF101")


def _label_text(record: Mapping[str, Any]) -> str:
    """Read the canonical positive label from raw or legacy UCF101 rows."""
...
def _negative_text(record: Mapping[str, Any]) -> str:
    """Read the optional negative label from raw or legacy UCF101 rows."""
...
def _text_side(text: str, trailing_newline: str) -> dict[str, Any]:
    ...
    return {
        "text": _apply_trailing_newline(normalized, trailing_newline, dataset_name="UCF101"),
        "media": [],
    }
...
def _build_video_media(record: Mapping[str, Any], *, num_frames: int) -> list[dict[str, Any]]:
    ...
    if isinstance(record.get("frames"), list):
        ...
        if not isinstance(video, (bytes, bytearray)):
            raise TypeError("UCF101 row is missing required video frames or raw video bytes.")
    ...
    if not images:
        raise ValueError("UCF101 row resolved to zero sampled video frames.")
    return [{"kind": "video", "content": images, "metadata": media_metadata}]


def transform_ucf101_train_sample(
    record: dict[str, Any],
    *,
    dataset_name: str,
    split: str,
    num_frames: int,
    config: Ucf101TransformConfig,
) -> dict[str, Any]:
    """Transform one joined UCF101 row into a TrainSample."""
...
def build_ucf101_train_default_transform(
    *,
    dataset_name: str,
    split: str,
    num_frames: int,
    ...
    """Build the pickle-safe default UCF101 transform."""

    return partial(
        transform_ucf101_train_sample,
        dataset_name=dataset_name,
        split=split,
        num_frames=num_frames,
...
@AutoDataset.register("ucf101_train")
class Ucf101TrainDataset(SampleLanceDataset):
    """UCF101 training dataset over raw split rows and joined video bytes."""
    ...
    def __init__(
        ...
        *,
        ...
        if transform is not None and not callable(transform):
            raise TypeError("Ucf101TrainDataset transform must be a callable SampleTransform or None.")
        ...
        if use_legacy_layout and not allow_raw_video:
            raise FileNotFoundError(
                f"Missing UCF101 pre-extracted frame table: {frame_units_path}. "
        ...
        if not frame_units_path.exists() and raw_path.exists():
            raise FileNotFoundError(f"Missing UCF101 pre-extracted frame table: {frame_units_path}")
        ...
        effective_transform = (
            ...
            else build_ucf101_train_default_transform(
                dataset_name=dataset_name,
                split=split,
                num_frames=num_frames,
    ...
    @classmethod
    def from_config(cls, config: Mapping[str, Any]) -> Ucf101TrainDataset:
        ...
        if transform_config is not None and not callable(transform_config):
            if not isinstance(transform_config, Mapping):
                raise TypeError("UCF101 config transform must be mapping, callable, or None.")
            kwargs.pop("transform")
            if "transform_kwargs" in kwargs:
                raise ValueError("UCF101 config cannot set both transform and transform_kwargs.")
...
__all__ = [
    ...
    "Ucf101TrainDataset",
    "build_ucf101_query_text",
    "build_ucf101_train_default_transform",
    "transform_ucf101_train_sample",
]
