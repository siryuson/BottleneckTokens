...
"""MMEB parser for VATEX video retrieval.

The parser keeps the archive prompt wording local so origin stays byte-aligned
with the original evaluation text. Canonical reuses the same defaults and only
applies the local normalization defined in this parser.
...
- Candidate: origin keeps inline video-token wording; canonical uses newline token
  separation and one trailing newline.
"""
...
DATASET_NAME = "VATEX"
PATH_REL = "video-tasks/VATEX"
MODALITY = "video"
TASK_TYPE = "video_retrieval"
# Dataset-local read columns keep the retrieval contract obvious.
QUERY_READ_COLUMNS = ("id", "enCap")
CANDIDATE_READ_COLUMNS = ("id", "video", "videoID")
# Archive-compatible prompt text stays beside the parser for easy review.
DEFAULT_TRANSFORM_KWARGS: dict[str, Any] = {
    "runtime_mode": "canonical",
    "num_frames": 8,
    "video_query_prefix": "Select a video that fits the description provided:",
...
def _normalize_transform_kwargs(transform_kwargs: Mapping[str, Any] | None = None) -> dict[str, Any]:
    # Layer caller overrides on top of the parser-local defaults.
...
def _compose_query_text(sample: dict[str, Any], *, transform_kwargs: dict[str, Any]) -> str:
    ...
    query_text = _normalize_query_text(sample.get("enCap"), runtime_mode=str(transform_kwargs["runtime_mode"]))
    return compose_parser_text_with_instruction(
        query_text,
        instruction=str(transform_kwargs["video_query_prefix"]),
        # Canonical query newline must stay parser-owned so removing the loader
        # patch later does not change the retrieval surface.
...
def _compose_candidate_text(*, transform_kwargs: dict[str, Any]) -> str:
    ...
    return compose_parser_text_with_instruction(
        ...
        # Canonical candidate block formatting is emitted here explicitly so it
        # no longer depends on the temporary loader-level newline append.
        token_separator="\n" if canonical else " ",
        append_trailing_newline=canonical,
    )
...
def _transform_candidate_item(
    ...
    *,
    ...
    if not media:
        raise ValueError("Missing required media for MMEB eval transform: dataset_name=VATEX, task_type=video_retrieval, role=candidate")
    ...
    metadata = extract_item_metadata(
        sample,
        consumed_fields={"id", "text", "images", "video", "image", "media_metadata", "enCap", "video_path"},
    )
    if metadata:
