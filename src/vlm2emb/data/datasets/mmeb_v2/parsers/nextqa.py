"""MMEB parser for NExTQA.

The archive prompt format is archive-style multiple choice:
- query text contains the question and five answer options;
- candidate text stays answer-only after conversion.
Canonical keeps the same wording and choice block, but parser-owned canonical
formatting moves the video token onto its own line and ensures the completed
prompt ends with one trailing newline.
"""
...
DATASET_NAME = "NExTQA"
PATH_REL = "video-tasks/NExTQA"
MODALITY = "video"
TASK_TYPE = "video_question_answer"
QUERY_READ_COLUMNS = ("id", "question", "a0", "a1", "a2", "a3", "a4", "video")
CANDIDATE_READ_COLUMNS = ("id", "text")
# NExTQA has its own runtime defaults even if the values match other QA parsers
# today. Canonical query layout is also parser-owned so the loader never has to
# infer where the visual token or final newline should go.
...
def _require_dataset_name(dataset_name: str) -> None:
    if dataset_name != DATASET_NAME:
        raise ValueError(f"Unsupported NExTQA parser dataset: {dataset_name}")
...
def _normalize_for_mode(text: str, *, mode: str) -> str:
    # Canonical owns query whitespace cleanup for NExTQA. Origin preserves the
    # archive surface exactly.
    if mode == "canonical":
...
def _normalize_candidate_for_mode(text: str, *, mode: str) -> str:
    # Canonical trims candidate-side whitespace artifacts left by conversion,
    # including trailing spaces and duplicated internal spaces.
    if mode == "canonical":
        return normalize_text_whitespace(text).rstrip("\n")
    return text
...
def _compose_query_text(sample: dict[str, Any], *, defaults: dict[str, Any]) -> str:
    ...
    options = [
        ...
        for field_name in ("a0", "a1", "a2", "a3", "a4")
    ]
    # Archive uses the standard archive-style choice template for NExTQA.
    ...
    # Origin keeps the archive inline token layout. Canonical keeps the exact
    # same wording and options, but the parser owns the standalone token line
    # and final newline instead of relying on a loader rewrite.
...
def build_query_transform(
    *,
    ...
    return partial(_build_nextqa_item, role="query", defaults=normalized)
...
def build_candidate_transform(
    *,
    ...
    _require_dataset_name(dataset_name)
    normalized = _normalize_transform_kwargs(transform_kwargs)
    return partial(_build_nextqa_item, role="candidate", defaults=normalized)
