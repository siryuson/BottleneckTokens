...
|------|-----|
| 原始名称 | QVHighlights |
| 论文 | QVHighlights: Detecting Moments and Highlights in Videos via Natural Language Queries（Lei et al., 2021）([arXiv](https://arxiv.org/abs/2107.09609)) |
| HuggingFace | [siyrus/BToks-QVHighlight](https://huggingface.co/datasets/siyrus/BToks-QVHighlight) |
| 许可证 | 参见原始数据集 |

### Schema
- `query`: 查询文本
- `clips_dir_path`/`video_id`: 候选片段来源
- `label_name`: 正样本片段标识
...
| 属性 | 值 |
|------|-----|
| 目录 | `MMEB-V2/video-tasks/QVHighlight/` |
| task_type | `video_moment_retrieval` |
| 评测模式 | local |

### Lance 布局
- `queries.lance`: 查询视频帧+文本（`id`, `text`, `images`）
