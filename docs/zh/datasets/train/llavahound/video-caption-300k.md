...
> ShareGPTVideo 视频字幕检索训练子集（caption_retrieval 模式），使用双表 Lance 结构组织视频帧与指令。
> 视频帧与 `conversations` 属于存储事实；`<video>` 到 `<|video_pad|>` 的替换属于 runtime transform policy。
...
| 表 | 关键字段 | 说明 |
|----|----------|------|
| 帧表 | video_id, frame_idx, image | 存储视频帧 |
| 指令表 | id, video, conversations | 存储视频对话指令 |

### 样本

**查询**: `<|video_pad|>...`
**目标**: `The video opens with a view of an indoor court marked with various lines, presumably for different s`
