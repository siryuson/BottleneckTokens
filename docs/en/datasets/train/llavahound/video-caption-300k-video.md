...
> ShareGPTVideo video caption retrieval training subset (video_retrieval mode), query is text description, positive sample is video frame sequence.
> Video frames and caption text are stored facts; video-retrieval prompt wording and `<|video_pad|>` injection are runtime transform policy.
...
### Schema
Uses the same original dual-table as `video-caption-300k`, only the training mapping mode is different.
...
| Config Key | `video_caption_300k-video` |
