...
> Video action recognition dataset focused on fine-grained human-object interaction and temporal relation understanding.
...
| Property | Value |
|------|-----|
| Original name | Something-Something V2 |
| Paper | The "Something Something" Video Database for Learning and Evaluating Visual Common Sense (Goyal et al., 2017) ([arXiv](https://arxiv.org/abs/1706.04261)) |
...
- `neg_text`: negative labels associated with the current sample (for local exhaustive evaluation)
...
- `neg_text`: candidate negative labels for each query
...
| Property | Value |
|------|-----|
| Directory | `MMEB-V2/video-tasks/SmthSmthV2/` |
| task_type | `video_classification` |
| Evaluation mode | local |

### Lance Layout
- `queries.lance`: video-frame query (`id`, `text`, `images`)
- `candidates.lance`: full class candidates (`id`, `text`, `images=[]`)
- `qrels.lance`: exhaustive relevance annotations (`mode=exhaustive`, with positive and negative candidate scores)
